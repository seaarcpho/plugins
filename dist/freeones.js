...
var _default$6 = decodeCodePoint;
 ...
 * Module dependencies
 */
var ElementType = __importStar(lib$a);
...
function formatAttributes(attributes, opts) {
    ...
    return Object.keys(attributes)
        ...
        return key + "=\"" + (opts.decodeEntities ? lib$9.encodeXML(value) : value.replace(/"/g, "&quot;")) + "\"";
    })
        .join(" ");
}
/**
...
var nth_check_1 = __importDefault(lib$5);
...
function compileUnsafe(selector, options, context) {
    var token = lib$7.parse(selector, options);
...
function filter(selector, elements, options) {
    if (options === void 0) { options = {}; }
    return DomUtils.uniqueSort(filterParsed(lib$7.parse(selector, options), elements, options));
}
exports.filter = filter;
/**
...
function filterParsed(selector, elements, options) {
    ...
    for (var _i = 0, filteredSelectors_1 = filteredSelectors; _i < filteredSelectors_1.length; _i++) {
        ...
        if (filteredSelector.some(lib$7.isTraversal)) {
            /*
...
function select(selector, root, options) {
    if (options === void 0) { options = {}; }
    var _a = helpers.groupSelectors(lib$7.parse(selector, options)), plain = _a[0], filtered = _a[1];
...
function findFilterElements(root, selector, options, queryForSelector, scopeContext) {
    ...
    var elemsNoLimit = sub.length === 0 && !Array.isArray(root)
        ...
        : sub.length === 0 || (sub.length === 1 && sub[0] === SCOPE_PSEUDO)
            ? Array.isArray(root)
                ? root
                : [root]
            : queryForSelector || sub.some(lib$7.isTraversal)
    ...
    var remainingHasTraversal = remainingSelector.some(lib$7.isTraversal);
...
function findElements(root, sel, options, limit) {
    ...
    var query = lib$4._compileToken(sel, options, root);
    var elems = lib$4.prepareContext(root, DomUtils, query.shouldTestNextSiblings);
    return DomUtils.find(function (node) { return DomUtils.isTag(node) && query(node); }, elems, true, limit);
}
function filterElements(elements, sel, options) {
    // @ts-expect-error TS seems to mess up the type here ¯\_(ツ)_/¯
    var query = lib$4._compileToken(sel, options);
...
Object.defineProperty(exports, "DomHandler", { enumerable: true, get: function () { return lib$1.DomHandler; } });
Object.defineProperty(exports, "DefaultHandler", { enumerable: true, get: function () { return lib$1.DomHandler; } });
// Helper methods
/**
...
function parseDocument(data, options) {
    var handler = new lib$1.DomHandler(undefined, options);
...
exports = module.exports = function parse(content, options, isDocument) {
  ...
  if (typeof content === 'string') {
    ...
    dom = useHtmlParser2
      ? lib.parseDocument(content, options)
    ...
    if (
      ...
      var root = new lib$1.Document(content);
...
function parseWithParse5(content, options, isDocument) {
  var parse = isDocument ? lib$2.parse : lib$2.parseFragment;
  ...
  var clone =
    'length' in dom
      ? Array.prototype.map.call(dom, function (el) {
          return lib$1.cloneNode(el, true);
        })
      : [lib$1.cloneNode(dom, true)];

  // Add a root node around the cloned nodes
  var root = new lib$1.Document(clone);
...
var domEach = utils.domEach;
var uniqueSort = lib.DomUtils.uniqueSort;
var isTag = utils.isTag;
...
exports.find = function (selectorOrHaystack) {
  ...
  return this._make(lib$3.select(selectorOrHaystack || '', elems, options));
};
...
exports.parentsUntil = function (selector, filter) {
  ...
  return this._make(
    filter ? lib$3.select(filter, parentNodes, this.options) : parentNodes
  );
};
...
exports.nextUntil = function (selector, filterSelector) {
  ...
  if (typeof selector === 'string') {
    untilNode = lib$3.select(selector, this.nextAll().get(), this.options)[0];
  } else if (selector && selector.cheerio) {
...
exports.prevUntil = function (selector, filterSelector) {
  ...
  if (typeof selector === 'string') {
    untilNode = lib$3.select(selector, this.prevAll().get(), this.options)[0];
  } else if (selector && selector.cheerio) {
    untilNodes = selector.get();
  } else if (selector) {
    untilNode = selector;
...
exports.filter = function (match, container) {
  ...
  if (typeof match === 'string') {
    elements = lib$3.filter(match, elements, container.options);
  } else {
...
exports.not = function (match, container) {
  ...
  if (typeof match === 'string') {
    matches = new Set(lib$3.filter(match, elements, this.options));
...
exports.text = function (str) {
  ...
  domEach(this, function (i, el) {
    ...
    var textNode = new lib$1.Text(str);
